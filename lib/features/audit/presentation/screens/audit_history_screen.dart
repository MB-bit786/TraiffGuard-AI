...
class _AuditHistoryScreenState extends ConsumerState<AuditHistoryScreen> {
  ...
  List<InvoiceModel> _getFilteredInvoices(List<InvoiceModel> invoices, AuditFilter filter) {
    return invoices.where((invoice) {
      // 1. Status Filter
      final matchesStatus = filter == AuditFilter.all || 
          (filter == AuditFilter.synced && invoice.status == InvoiceSyncStatus.synced) ||
          (filter == AuditFilter.offlineDraft && invoice.status == InvoiceSyncStatus.offlineDraft);
